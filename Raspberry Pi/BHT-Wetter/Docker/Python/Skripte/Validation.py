...
try:
  pay = subscribe(topic="Validation",hostname= host, username = user, password = pw,timeout= 300).payload
  print(pay)
except:
  print('Message empty')
...
temp = "{:.2f}".format(float(pay_ob["Temp"]))
pres = "{:.2f}".format(float(pay_ob["Hum"]))
hum = "{:.2f}".format(float(pay_ob["Pres"]))

#UTC Zeit in Localtime umwandeln
date_time_utc = datetime.datetime.strptime(str(pay_ob["Time"]),"%Y/%m/%d %H:%M:%S").replace(tzinfo=pytz.UTC)
...
sql = "INSERT INTO BME280_Validation (Temp,Hum,Pres,StatusTime) VALUES (%s,%s,%s,%s)"
